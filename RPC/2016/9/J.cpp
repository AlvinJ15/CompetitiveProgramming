...
long long DP(int index,int cant){
  ...
  if(cant==1)return memo[index][cant]=max(DP(index-1,1),(long long)arr[index]);
  if(memo[index][cant]!=-1)return memo[index][cant];
  memo[index][cant]=max(arr[index]+DP(index-1,cant-1),max(DP(index-1,min(cant,(index)/2)+(!(index-1))),DP(index,cant-1)));
